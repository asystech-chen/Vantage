...
vantage-doh-mode-fallback =
    .label = 优先（DoH 不可用时回退系统 DNS）
vantage-doh-mode-only =
    .label = 仅 DoH（不回退）
vantage-doh-description = 开启后：DNS 查询通过加密的 HTTPS 发送至下方所选的 DNS 供应商解析（默认阿里 AliDNS）。DoH 不可用时自动回退系统 DNS，内网/VPN 访问不受影响。
vantage-doh-provider-label = DNS 供应商：
vantage-doh-provider-custom =
    .label = 自定义…
vantage-doh-custom-label = 自定义 DoH 地址：
...
# 下载提速
vantage-download-speed-heading = 下载提速
...
# ---- 指纹随机化（FPP）----
vantage-fpp-heading = 指纹随机化
vantage-fpp-checkbox =
    .label = 启用指纹随机化（FPP）
vantage-fpp-preset-label = 预设：
...
# ---- 微软账户同步 ----
vantage-msa-heading = 微软账户同步
vantage-msa-description = 使用微软账户将浏览器配置（书签、历史、扩展、设置）备份到您自己 OneDrive 的 Vantage 私有应用文件夹，并可在任意设备上恢复。登录密码不会包含在备份中。数据仅保存在您的微软账户里——Vantage 不运营任何同步服务器。
vantage-msa-clientid-label = 应用程序(客户端) ID：
vantage-msa-clientid-hint = 需要在 Microsoft Entra 管理中心免费注册一个应用：开启「允许公共客户端流」，添加委托权限 User.Read 和 Files.ReadWrite.AppFolder，然后把「应用程序(客户端) ID」粘贴到此处。个人微软账户使用租户 consumers（默认；可通过 about:config 的 vantage.msaSync.tenant 调整）。
vantage-msa-need-clientid = 请先填写应用程序(客户端) ID（点击右侧帮助图标查看配置说明）。
vantage-msa-need-signin = 请先登录微软账户。
vantage-msa-signin-button = 登录微软账户…
vantage-msa-signout-button = 退出登录
vantage-msa-pending-instructions = 已打开微软登录页，请在页面中输入此代码：
vantage-msa-open-verify-button = 重新打开登录页
vantage-msa-cancel-button = 取消
vantage-msa-waiting = 等待您完成登录…
vantage-msa-signin-success = 登录成功。
vantage-msa-signedout = 已退出微软账户。
vantage-msa-account = 已登录：{ $account }
vantage-msa-lastsync = 上次云端备份：{ $time }
vantage-msa-neversync = 尚未进行过云端备份
vantage-msa-upload-button = 备份到云端
vantage-msa-restore-button = 从云端恢复…
vantage-msa-upload-confirm-title = 备份到微软账户
vantage-msa-upload-confirm = 将打包当前配置并上传到您 OneDrive 的 Vantage 私有应用文件夹。备份包含敏感数据：Cookie（网站登录状态）、浏览历史和设置；登录密码已排除。确定继续吗？
vantage-msa-upload-working = 正在打包并上传…
vantage-msa-upload-progress = 正在上传… { $pct }%
vantage-msa-upload-success = 已备份到您的 OneDrive。
vantage-msa-upload-fail = 上传失败：{ $error }
vantage-msa-restore-working = 正在下载云端备份…
vantage-msa-restore-none = 该账户下没有找到云端备份。
vantage-msa-restore-confirm-title = 从微软账户恢复
vantage-msa-restore-confirm = 云端备份（{ $time } 由「{ $device }」创建）将覆盖当前配置并重启浏览器。恢复前会自动在本地备份当前配置。确定继续吗？
vantage-msa-restore-unknown = 未知
vantage-msa-restore-fail = 恢复失败：{ $error }
vantage-msa-error-config = 应用程序(客户端) ID 缺失或无效。
vantage-msa-error-network = 网络错误：无法连接微软服务器。
vantage-msa-error-auth = 身份验证失败：{ $error }。可能需要重新登录。
vantage-msa-error-expired = 登录代码已过期，请重试。
vantage-msa-error-cancelled = 已取消登录。
