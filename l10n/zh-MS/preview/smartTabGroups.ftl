...
tab-group-editor-title-suggest = 建议的标签页
...
tab-group-editor-suggestions-header = 建议的标签页

tab-group-suggestions-loading-header = 建议的标签页
...
tab-group-editor-no-tabs-found-message = 打开相似的标签页，AI 将建议一个名称并将它们与此窗口中的标签页分组。

# Message is shown to the user whenever there are smart tags suggest
tab-group-editor-information-message = AI 根据此窗口中相似的打开标签页建议名称和标签页。

# Title appears when user is shown the opt-in flow to use smart tab grouping
tab-group-suggestions-optin-title = 建议更多我的标签页

# Message appears when user is shown the opt-in flow to use smart tab grouping
tab-group-suggestions-optin-message = { -brand-short-name } 使用 AI 读取您打开的标签页的标题和描述，以<a data-l10n-name="support">建议更多标签页和组名称</a>。此操作将在您的设备上执行。

tab-group-suggestions-optin-title-download = 正在查找建议…

tab-group-suggestions-optin-message-download = 此操作可能需要片刻时间。

tab-group-suggestions-optin-message-footer = 在<a data-l10n-name="settings">设置</a>中自定义 AI
