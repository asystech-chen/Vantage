...
sync-disconnect-dialog-body = { -brand-product-name } 将停止【电影、电视】你的帐户除了将不删除任何的的你的浏览论据在这装置.
sync-disconnect-dialog-button = 断开

fxa-signout-dialog-title2 = 正负号外面的的你的帐户?
fxa-signout-dialog-body = synchronization 的缩略词论据将保持在你的帐户.
fxa-signout-dialog2-button = 正负号外面的
fxa-signout-dialog2-checkbox = 删除论据从这装置 (密码, 历史记录, 书签, 及其他.)
...
fxa-menu-send-tab-to-device-description = 发送一个标签页立即地到任何的装置你是有符号的在在.
...
sync-profile-different-account-title = 帐户界限到达为了这提问档
sync-profile-different-account-header = 这提问档曾是先前地 synchronization 的缩略词到一个不同的帐户

# Variables:
#   $acctEmail (String) - Email of the account signing into sync.
sync-profile-different-account-description = 到保持你的论据有组织的和无虑的, 每个 { -brand-product-name } 提问档能仅是 synchronization 的缩略词到一(个) 帐户. 到正负号在使用 { $acctEmail }, 创建新配置文件.
...
sync-profile-different-account-description-merge = 到保持你的论据有组织的和无虑的, 我们推荐创建一个新的提问档到正负号在使用 { $acctEmail }. 如果你选取到继续到同步的在这提问档, 论据从两者的帐户将是永久性的合并在“{ $profileName }”.
...
sync-account-in-use-description-merge = { $acctEmail } 是有符号的在到那“{ $otherProfile }”提问档在这计算机. 【电影、电视】那“{ $currentProfile }”提问档将永久性的 (使)联合论据从两者的提问档, 如此的作为密码和书签.

# Variables:
#   $profileName (String) - Name of the profile to switch to
sync-button-switch-profile = 开关到“{ $profileName }”
sync-button-create-profile = 创建一个新的提问档
sync-button-sync-and-merge = 同步的和合并论据
# Variables:
#   $profileName (String) - Name of the profile to switch to
