...
migration-wizard-selection-header = 引入浏览器资料
migration-wizard-selection-list = 选择那资料 (=you had)(=you would) 相似的到引入.
...
migration-wizard-migrator-display-name-chrome-dev = 铬发展（develop）
...
migration-no-permissions-instructions = 到继续导入资料从另外的浏览器, 授权命令 { -brand-short-name } 访问到它的提问档文件夹.
...
migration-all-available-data-label = 引入所有的可利用的资料
migration-no-selected-data-label = 无资料被选的为了引入
migration-selected-data-label = 引入被选的资料
...
migration-passwords-from-file-no-valid-data = 那文件不包含任何的有确实根据的密码资料. 拾取另外的文件.
...
migration-bookmarks-from-file-no-valid-data = 那文件不包含任何的书签资料. 拾取另外的文件.
...
migration-wizard-import-browser-no-browsers = { -brand-short-name } (=could not)不能发现任何的程序那包含书签, 历史记录或密码资料.
migration-wizard-import-browser-no-resources = 那里曾是一个错误. { -brand-short-name } 不能发现任何的资料到引入从那浏览器提问档.
...
migration-list-autofill-label = 自动填充资料
migration-list-payment-methods-label = 付款方法

##

migration-wizard-progress-header = 导入资料

# This header appears in the final page of the migration wizard only if
# all resources were imported successfully.
migration-wizard-progress-done-header = 资料输入成功

# This header appears in the final page of the migration wizard if only
# some of the resources were imported successfully. This is meant to be
# distinct from migration-wizard-progress-done-header, which is only shown
# if all resources were imported successfully.
migration-wizard-progress-done-with-warnings-header = 资料引入完全的
...
migration-safari-password-import-step1 = 在旅行, 打开“旅行”菜单和去到参数选择 > 密码
