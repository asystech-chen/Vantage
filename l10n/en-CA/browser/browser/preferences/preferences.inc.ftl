...
vantage-doh-mode-label = DoH mode:
vantage-doh-mode-fallback =
    .label = Prefer DoH (fall back to system DNS when unavailable)
vantage-doh-mode-only =
    .label = DoH only (no fallback)
...
# ---- Fingerprint Randomization (FPP) ----
vantage-fpp-heading = Fingerprint Randomization
vantage-fpp-checkbox =
    .label = Enable fingerprint randomization (FPP)
vantage-fpp-preset-label = Preset:
...
vantage-msa-heading = Microsoft Account Sync
vantage-msa-description = Back up your profile (bookmarks, history, extensions, settings) to the private Vantage app folder in your own OneDrive with a Microsoft account, and restore it on any device. Login passwords are never included. Your data stays in your Microsoft account — Vantage does not run any sync server.
vantage-msa-clientid-label = Application (client) ID:
vantage-msa-clientid-hint = Requires a free app registration in the Microsoft Entra admin center: enable "Allow public client flows", add the delegated permissions User.Read and Files.ReadWrite.AppFolder, then paste the Application (client) ID here. Personal Microsoft accounts use the "consumers" tenant (default; adjustable via vantage.msaSync.tenant in about:config).
vantage-msa-need-clientid = Enter your Application (client) ID first (click the help icon for setup instructions).
vantage-msa-need-signin = Sign in with your Microsoft account first.
vantage-msa-signin-button = Sign in with Microsoft…
vantage-msa-signout-button = Sign out
vantage-msa-pending-instructions = A Microsoft sign-in page has been opened. Enter this code there:
vantage-msa-open-verify-button = Reopen sign-in page
vantage-msa-cancel-button = Cancel
vantage-msa-waiting = Waiting for you to complete sign-in…
vantage-msa-signin-success = Signed in successfully.
vantage-msa-signedout = Signed out of your Microsoft account.
vantage-msa-account = Signed in as { $account }
vantage-msa-lastsync = Last cloud backup: { $time }
vantage-msa-neversync = No cloud backup yet
vantage-msa-upload-button = Back Up to Cloud
vantage-msa-restore-button = Restore from Cloud…
vantage-msa-upload-confirm-title = Back Up to Microsoft Account
vantage-msa-upload-confirm = Your profile will be packed and uploaded to the private Vantage app folder in your OneDrive. It contains sensitive data: cookies (website login sessions), browsing history and settings. Login passwords are excluded. Continue?
vantage-msa-upload-working = Packing and uploading…
vantage-msa-upload-progress = Uploading… { $pct }%
vantage-msa-upload-success = Backup uploaded to your OneDrive.
vantage-msa-upload-fail = Upload failed: { $error }
vantage-msa-restore-working = Downloading cloud backup…
vantage-msa-restore-none = No cloud backup was found for this account.
vantage-msa-restore-confirm-title = Restore from Microsoft Account
vantage-msa-restore-confirm = The cloud backup (created { $time } on "{ $device }") will replace your current configuration and restart the browser. A local backup of your current profile is created automatically. Continue?
vantage-msa-restore-unknown = unknown
vantage-msa-restore-fail = Restore failed: { $error }
vantage-msa-error-config = The Application (client) ID is missing or invalid.
vantage-msa-error-network = Network error: cannot reach the Microsoft servers.
vantage-msa-error-auth = Authentication failed: { $error }. You may need to sign in again.
vantage-msa-error-expired = The sign-in code expired. Please try again.
vantage-msa-error-cancelled = Sign-in cancelled.
